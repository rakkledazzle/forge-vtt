...
import { Card, Btn, Modal, FormField, Grid, Badge, Section, SearchInput, EmptyState, Tabs } from './UI';
...
function CampaignForm({ campaign, onSave, onClose }) {
  ...
  function addPlayer() { const p=prompt('Player name:'); if(p) setF(x=>({...x,players:[...x.players,{name:p,character:''}]})); }
  function addNPC() { setF(x=>({...x,npcs:[...x.npcs,{id:Date.now(),name:'',role:'',notes:'',attitude:'Neutral'}]})); }
  function addQuest() { setF(x=>({...x,quests:[...x.quests,{id:Date.now(),title:'',status:'Active',description:'',reward:''}]})); }
  function addLocation() { setF(x=>({...x,locations:[...x.locations,{id:Date.now(),name:'',type:'',description:'',visited:false}]})); }
  ...
  return (
    <div>
      ...
      <Section title="Players" action={<Btn size='sm' variant='ghost' onClick={addPlayer}>+ Add</Btn>}>
        {f.players.map((p,i)=>(
          <div key={i} style={{ display:'flex', gap:'0.5rem', marginBottom:'0.4rem', alignItems:'center' }}>
            <input value={p.name} onChange={e=>setF(x=>({...x,players:x.players.map((pl,j)=>j===i?{...pl,name:e.target.value}:pl)}))} placeholder="Player name" />
            <input value={p.character||''} onChange={e=>setF(x=>({...x,players:x.players.map((pl,j)=>j===i?{...pl,character:e.target.value}:pl)}))} placeholder="Character name" />
            <Btn size='sm' variant='danger' onClick={()=>setF(x=>({...x,players:x.players.filter((_,j)=>j!==i)}))}>×</Btn>
          </div>
        ))}
      </Section>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
        <div style={{ flex:1 }}>
          <div style={{ display:'flex', gap:'0.5rem', alignItems:'center', marginBottom:'0.35rem' }}>
            <input value={npc.name} onChange={e=>onUpdate({...npc,name:e.target.value})} placeholder="NPC Name" style={{ fontFamily:"'Cinzel',serif", fontSize:'0.9rem', background:'transparent', border:'none', borderBottom:'1px solid var(--border)', borderRadius:0, padding:'0.1rem 0.2rem', width:'150px' }} />
          ...
          <input value={npc.role||''} onChange={e=>onUpdate({...npc,role:e.target.value})} placeholder="Role / Occupation" style={{ fontSize:'0.8rem', color:'var(--text-muted)', marginBottom:'0.35rem' }} />
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
        <div style={{ flex:1 }}>
          <div style={{ display:'flex', gap:'0.5rem', alignItems:'center', marginBottom:'0.4rem' }}>
            <input value={quest.title} onChange={e=>onUpdate({...quest,title:e.target.value})} placeholder="Quest title" style={{ fontFamily:"'Cinzel',serif", background:'transparent', border:'none', borderBottom:'1px solid var(--border)', borderRadius:0, padding:'0.1rem', flex:1 }} />
          ...
          <textarea value={quest.description||''} onChange={e=>onUpdate({...quest,description:e.target.value})} rows={2} placeholder="Quest description, objectives…" style={{ fontSize:'0.82rem', marginBottom:'0.3rem' }} />
          <input value={quest.reward||''} onChange={e=>onUpdate({...quest,reward:e.target.value})} placeholder="Reward: 500 gp, Bag of Holding…" style={{ fontSize:'0.8rem', color:'var(--gold)' }} />
        </div>
        <Btn size='sm' variant='danger' onClick={onDelete} style={{ marginLeft:'0.5rem' }}>×</Btn>
      </div>
    </Card>
...
export default function CampaignManager({ campaigns, onSave, onDelete }) {
    ...
    return (
      <div style={{ maxWidth:'900px', margin:'0 auto' }}>
        <div style={{ display:'flex', alignItems:'center', gap:'1rem', marginBottom:'1.5rem' }}>
          ...
          <Btn variant='ghost' size='sm' onClick={()=>{setEditing(campaign);setModal(true);}}>Edit</Btn>
          <Btn variant='danger' size='sm' onClick={()=>{if(window.confirm('Delete this campaign?')){onDelete(campaign.id);setActive(null);}}}>Delete</Btn>
        </div>

        <Tabs tabs={[{id:'overview',label:'Overview'},{id:'npcs',label:`NPCs (${(campaign.npcs||[]).length})`},{id:'quests',label:`Quests (${(campaign.quests||[]).length})`},{id:'sessions',label:'Session Notes'},{id:'world',label:'World'}]} active={tab} onChange={setTab} />
        ...
        {tab==='overview' && (
          <div className="animate-in">
            <Grid cols={2} gap='1.5rem'>
              ...
              <Card>
                <h4 style={{ color:'var(--gold)', marginBottom:'0.75rem' }}>Party</h4>
                {(campaign.players||[]).length===0?<div style={{color:'var(--text-muted)',fontSize:'0.85rem'}}>No players added.</div>:
                  campaign.players.map((p,i)=>(
                    <div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'0.4rem 0', borderBottom:'1px solid var(--border)', fontSize:'0.85rem' }}>
                      <span>{p.name}</span>
                      <span style={{ color:'var(--text-muted)' }}>{p.character}</span>
                    </div>
                  ))
                }
              </Card>
            ...
            <div style={{ marginTop:'1.5rem' }}>
              <Section title="Active Quests">
                {(campaign.quests||[]).filter(q=>q.status==='Active').length===0?<div style={{color:'var(--text-muted)',fontSize:'0.85rem'}}>No active quests.</div>:
                  (campaign.quests||[]).filter(q=>q.status==='Active').map(q=>(
                    <div key={q.id} style={{ padding:'0.5rem', borderLeft:'3px solid var(--emerald)', paddingLeft:'0.75rem', marginBottom:'0.5rem' }}>
        ...
        {tab==='npcs' && (
          <div className="animate-in">
            ...
            {(campaign.npcs||[]).length===0?<EmptyState icon='🧑' title='No NPCs yet' desc='Add important NPCs to track their attitudes and notes.' />:
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:'0.75rem' }}>
                {(campaign.npcs||[]).map(n=>(
                  <NPCCard key={n.id} npc={n} onUpdate={updateNPC} onDelete={()=>updateCampaign({npcs:(campaign.npcs||[]).filter(x=>x.id!==n.id)})} />
                ))}
              </div>
            }
          </div>
        ...
        {tab==='quests' && (
          <div className="animate-in">
            ...
            {(campaign.quests||[]).length===0?<EmptyState icon='📜' title='No quests yet' desc='Track your active and completed quests here.' />:
              <div style={{ display:'flex', flexDirection:'column', gap:'0.6rem' }}>
                {(campaign.quests||[]).map(q=>(
                  <QuestCard key={q.id} quest={q} onUpdate={updateQuest} onDelete={()=>updateCampaign({quests:(campaign.quests||[]).filter(x=>x.id!==q.id)})} />
                ))}
              </div>
            }
          </div>
        ...
        {tab==='sessions' && (
          <div className="animate-in">
            ...
            {(campaign.sessions||[]).length===0?<EmptyState icon='📓' title='No sessions logged' desc='Record what happened each session.' />:
                ...
                <Card key={s.id} style={{ marginBottom:'0.75rem' }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'0.5rem' }}>
                    <div>
                      ...
                      <input value={s.title||''} onChange={e=>updateCampaign({sessions:(campaign.sessions||[]).map(x=>x.id===s.id?{...x,title:e.target.value}:x)})} placeholder="Session title" style={{ background:'transparent', border:'none', borderBottom:'1px solid var(--border)', borderRadius:0, fontFamily:"'Cinzel',serif", fontSize:'0.9rem', color:'var(--gold)' }} />
                    </div>
                    <div style={{ display:'flex', gap:'0.5rem', alignItems:'center' }}>
                      <span style={{ fontSize:'0.78rem', color:'var(--text-muted)' }}>{s.date}</span>
                      <Btn size='sm' variant='danger' onClick={()=>updateCampaign({sessions:(campaign.sessions||[]).filter(x=>x.id!==s.id)})}>×</Btn>
                    </div>
                  </div>
                  <textarea value={s.summary||''} onChange={e=>updateCampaign({sessions:(campaign.sessions||[]).map(x=>x.id===s.id?{...x,summary:e.target.value}:x)})} rows={4} placeholder="What happened this session? Major events, decisions, discoveries…" style={{ fontSize:'0.85rem' }} />
        ...
        {tab==='world' && (
          <div className="animate-in">
            <Section title="World & Setting Notes">
              <textarea value={campaign.worldNotes||''} onChange={e=>updateCampaign({worldNotes:e.target.value})} rows={8} placeholder="History, gods, factions, geography, lore…" />
            </Section>
            <Section title="Locations" action={<Btn size='sm' variant='ghost' onClick={()=>updateCampaign({locations:[...(campaign.locations||[]),{id:Date.now(),name:'',type:'',description:'',visited:false}]})}>+ Add</Btn>}>
              {(campaign.locations||[]).map(l=>(
                <div key={l.id} style={{ display:'flex', gap:'0.5rem', marginBottom:'0.5rem', alignItems:'center' }}>
                  <input type="checkbox" checked={l.visited||false} onChange={e=>updateCampaign({locations:(campaign.locations||[]).map(x=>x.id===l.id?{...x,visited:e.target.checked}:x)})} style={{width:'auto'}} title="Visited" />
                  <input value={l.name||''} onChange={e=>updateCampaign({locations:(campaign.locations||[]).map(x=>x.id===l.id?{...x,name:e.target.value}:x)})} placeholder="Location name" style={{ flex:1 }} />
                  <input value={l.type||''} onChange={e=>updateCampaign({locations:(campaign.locations||[]).map(x=>x.id===l.id?{...x,type:e.target.value}:x)})} placeholder="Type (city, dungeon…)" style={{ flex:1 }} />
                  <Btn size='sm' variant='danger' onClick={()=>updateCampaign({locations:(campaign.locations||[]).filter(x=>x.id!==l.id)})}>×</Btn>
                </div>
              ))}
            </Section>
          </div>
  ...
  return (
    <div style={{ maxWidth:'900px', margin:'0 auto' }}>
      ...
      {campaigns.length === 0 ? (
        <EmptyState icon='🗺️' title='No campaigns yet' desc='Start your first campaign to track NPCs, quests, sessions, and lore.' action={<Btn variant='primary' onClick={()=>setModal(true)}>Create Campaign</Btn>} />
      ) : (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:'1rem' }}>
          {campaigns.map(c=>(
            <Card key={c.id} onClick={()=>setActive(c.id)} style={{ cursor:'pointer' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'0.5rem' }}>
                <h3 style={{ fontFamily:"'Cinzel',serif", color:'var(--gold-light)', fontSize:'1rem' }}>{c.name}</h3>
              ...
              <div style={{ display:'flex', gap:'0.5rem', flexWrap:'wrap' }}>
                {c.players?.length>0 && <Badge color='muted'>{c.players.length} players</Badge>}
                {c.sessions?.length>0 && <Badge color='muted'>{c.sessions.length} sessions</Badge>}
                {(c.quests||[]).filter(q=>q.status==='Active').length>0 && <Badge color='gold'>{(c.quests||[]).filter(q=>q.status==='Active').length} active quests</Badge>}
              </div>
            </Card>
          ))}
        </div>
